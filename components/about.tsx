    ...
    <section id="about" className="min-h-screen flex flex-col justify-center py-8 px-4 sm:px-6 lg:px-8 bg-muted/30">
      <div className="container mx-auto max-w-5xl">
        <div className="text-center mb-8">
          <h2 className="text-3xl sm:text-4xl font-bold text-foreground mb-2 animate-fade-in-up">About Me</h2>
        ...
        <div className="grid md:grid-cols-2 gap-8 items-center mb-8">
          <div className="animate-fade-in-up">
            <div className="aspect-square rounded-xl bg-gradient-to-br from-primary/20 to-secondary/20 flex items-center justify-center max-w-xs mx-auto">
              <img src="/profile.jpg" alt="Profile" className="rounded-xl w-full h-full object-cover" />
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {skills.map((skill, index) => (
            <Card
              key={index}
              className="p-4 hover:shadow-lg transition-all duration-300 hover:-translate-y-1 animate-fade-in-up border-border/50 text-center"
              style={{ animationDelay: `${index * 100}ms` }}
            >
              <skill.icon className="h-8 w-8 text-primary mb-3 mx-auto" />
